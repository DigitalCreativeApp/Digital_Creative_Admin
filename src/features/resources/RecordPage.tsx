...
export function RecordPage(){
 ...
 {editing?<RecordEditor resource={resource} data={record.data} busy={busy} onSave={save} onCancel={()=>setEditing(false)}/>:<dl className="record">{Object.entries(record.data).map(([name,value])=><div key={name}><dt>{fieldLabel(name)}</dt><dd><DataValue value={value}/></dd></div>)}</dl>}
