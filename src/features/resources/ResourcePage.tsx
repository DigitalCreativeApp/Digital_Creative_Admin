...
export function ResourcePage() {
  ...
  async function create(values: Record<string, unknown>) { setBusy(true); setError(''); try { await adminService.create(key, values); setCreating(false); setNotice('Đã tạo bản ghi mới.'); load(); } catch (e) { setError(e instanceof Error ? e.message : 'Không thể tạo bản ghi.'); } finally { setBusy(false); } }

  return <>
    <div className="page-title"><div><p>RESOURCE / {key.toUpperCase()}</p><h1>{resource?.name || key}</h1></div><div className="actions">{resource?.canCreate && <button onClick={() => setCreating(x => !x)}>{creating ? 'Đóng biểu mẫu' : '+ Tạo mới'}</button>}<span>{data ? `${data.total.toLocaleString('vi-VN')} bản ghi` : resource?.table}</span></div></div>
    {creating && resource && <div className="create-panel"><h2>Tạo {resource.name}</h2><RecordEditor resource={resource} data={{}} busy={busy} onSave={create} onCancel={() => setCreating(false)}/></div>}
