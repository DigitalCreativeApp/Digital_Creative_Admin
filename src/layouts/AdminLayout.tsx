...
export function AdminLayout() {
  ...
  return <div className={`shell ${compact ? 'sidebar-compact' : ''}`}>
    ...
    <div className="workspace">
      <header className="topbar"><button className="menu" onClick={() => setMobileOpen(true)} aria-label="Mở menu"><AppIcon name="menu"/></button><div className="breadcrumb"><span>Quản trị</span><AppIcon name="chevron"/>{activeResource ? <strong>{resourceLabel(activeResource.key,activeResource.name)}</strong> : <strong>{location.pathname.startsWith('/settings/') ? 'Cấu hình phí' : location.pathname.startsWith('/admin/withdrawals') ? 'Yêu cầu rút tiền' : location.pathname.startsWith('/admin/agreement') ? 'Pháp lý & thỏa thuận' : 'Tổng quan'}</strong>}</div><div className="topbar-status"><span className="online-dot"/><span>Hệ thống hoạt động</span></div><button className="topbar-logout" onClick={() => void signOut()}><AppIcon name="logout"/><span>Đăng xuất</span></button></header>
